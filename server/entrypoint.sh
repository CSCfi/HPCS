...
# TODO: redefine proper "socket_path" more clearly, otherwise returns two lines
agent_socket_path=$(grep "^  socket_path" /tmp/agent.conf | cut -d "=" -f2 | cut -d '"' -f2)
...
until [ -e "${agent_socket_path}" ]; do
	printf "%b[LUMI-SD][HPCS server] Spire workload api socket doesn't exist, waiting 10 seconds %b" "${RED}" "${NC}\n"
	sleep 10
done
