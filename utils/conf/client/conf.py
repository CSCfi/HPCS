...
def parse_configuration(path: str):
    ...
    if not "supercomputer" in config:
        raise NoSectionError(
            "supercomputer section missing in configuration file, aborting"
        )

    if not "spire-server" in config:
        raise NoSectionError(
            "hpcs-server section missing in configuration file, aborting"
        )
    ...
    if (
        not "address" in config["supercomputer"]
        or not "username" in config["supercomputer"]
    ):
        raise NoOptionError(
            "'spire-server' section is incomplete in configuration file, aborting"
        )
    ...
    if not "url" in config["hpcs-server"]:
        raise NoOptionError(
            "'hpcs-server' section is incomplete in configuration file, aborting"
        )

    if not "url" in config["vault"]:
        raise NoOptionError(
            "'vault' section is incomplete in configuration file, aborting"
        )
